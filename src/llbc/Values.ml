...
type abs_kind =
  | FunCall of (FunCallId.id * RegionGroupId.id * bool)
      (** The abstraction was introduced because of a function call.
          ...
          of regions to which they are associated), and whether the region group
          is stateful. This is not used by the symbolic execution: this is only
          used for pretty-printing and debugging in the symbolic AST, generated
