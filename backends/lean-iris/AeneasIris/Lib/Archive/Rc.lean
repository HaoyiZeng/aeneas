...
/-! # The ITree `Rc`: reference counting over a plain heap

Archived.  `ArcImpl` is the implementation clients use; this one keeps the
counters on the plain heap, and supplies its `ArcAPI` as a `def` so that
instance search never picks it. -/

unseal Aeneas.Std.Result

namespace AeneasIris.RcImpl
