...
theorem Vec.index_mut_slice_index {α : Type} (v : Vec α) (i : Usize) :
  ...
  simp only [Vec.index_mut, Vec.index_mut_usize, Slice.index_mut_usize,
             Bind.bind, Std.bind_assoc_eq, Std.pure_bind]
  have hset : (fun o' => ({ slice := v.slice.set i o' } : Vec α)) = Vec.set v i := by
    funext o'
    simp [Vec.set, Slice.set, Slice.setAtNat, Vec.from, Vec.val, Slice.from]
  simp only [hset, Vec.index_usize, Slice.index_usize, Vec.val, pure]
  rfl
...
def alloc.vec.Vec.extend_from_slice {T : Type} (cloneInst : core.clone.Clone T)
  ...
  if h : v.length + s.length ≤ Usize.max then do
    /- `Vec` and `Slice` used to be the same subtype of `List`, so the failure of
       the clone could be returned directly.  They are now distinct structures
       (`ListN`-backed, for positivity), so the error has to be re-typed.  Under
       the ITree `Result` a `.vis` carries a continuation, which would have to be
       remapped; binding and re-checking the length is the same function with one
       extra branch, and that branch is unreachable given `h`. -/
    let s' ← Slice.clone cloneInst.clone s
    if h2 : (v.val ++ s'.val).length ≤ Usize.max then ok (.from _ h2)
    else fail .panic
  else fail .panic
